...
<body>
	...
	<form id="boardForm" action="updateBoard" method="post">
		...
		<input type="hidden" name="payOption" value="${board.payOption}">
		<input type="hidden" name="loginId" value="${loggedInUserId}">
		<input type="hidden" name="loginRank" value="${loggedInUserRank}">
		...
		<c:if test="${not empty board}">
			<table border="1">
				...
				<tr>
					<th>제목</th>
					<td><c:choose>
							<c:when test="${board.payOption eq 'save'}">
		...
		<div class="button-container">
			<button type="submit" id="saveButton" name="save"
				style="${board.payOption eq 'save' ? 'display: inline-block;' : 'display: none;'}">임시저장</button>
			<button type="submit" id="approvalButton" name="approval"
				style="${(board.payOption eq 'save' && (loggedInUserRank eq '사원' || loggedInUserRank eq '대리')) || (board.payOption eq 'wait' && loggedInUserRank eq '과장') || (board.payOption eq 'checking' && loggedInUserRank eq '부장') ? 'display: inline-block;' : 'display: none;'}">결재</button>
		 <button type="submit" id="rejectButton" name="reject"
            style="${board.payOption eq 'wait' && loggedInUserRank eq '과장' ? 'display: inline-block;' : 'display: none;'} ">반려</button>
