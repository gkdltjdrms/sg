	...
	public String home(Locale locale, Model model) {
	   ...
	   @RequestMapping("nexaConn")
	    public void nexaList(HttpServletRequest request, HttpServletResponse response) throws PlatformException {
	        try {
	            // MyBatis를 통해 board_study 테이블의 데이터를 조회
	            List<Map<String, Object>> resultList = sqlSession.selectList("mapper.BoardMapper.boardList");
	            ...
	            for (Map<String, Object> row : resultList) {
	                ...
	                ds.set(newRow, "columnName1", row.get("columnName1"));
	                ds.set(newRow, "columnName2", row.get("columnName2"));
	                // 필요한 만큼 열 추가
	            }
			 ...
			 public String search(@RequestParam("id") String id,
				 ...
				 User user = postService.getUserInfo(id);
				   ...
				   System.out.println(userRank+"이 직급으로 검색");
			        List<Board> boardList = new ArrayList<Board>(); // Initialize with an empty list
