	...
	<form id="boardForm" action="updateBoard" method="post">
        ...
        style="${(board.payOption eq 'save') or (board.payOption eq 'reject' and board.id eq user.id) ? 'display: inline-block;' : 'display: none;'}">임시저장</button>
			<button type="submit" id="approvalButton" name="action"
				value="approval"
			  style="${(board.payOption eq 'save' or (board.payOption eq 'reject' and board.id eq user.id)) || (board.payOption eq 'wait' && loggedInUserRank eq '과장') || (board.payOption eq 'checking' && loggedInUserRank eq '부장') ? 'display: inline-block;' : 'display: none;'}">결재</button>
