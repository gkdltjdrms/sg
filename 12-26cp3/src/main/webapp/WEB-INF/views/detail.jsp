	...
	<form id="boardForm" action="updateBoard" method="post">
		...
		<c:if test="${not empty board}">
			<table border="1">
				...
				<tr>
					...
					<td><c:choose>
							<c:when
								test="${board.payOption eq 'save' or board.payOption eq 'reject'}">
		...
		<div class="button-container">
			<button type="submit" id="saveButton" name="action" value="save"
				style="${board.payOption eq 'save' or board.payOption eq 'reject' or board.payOption eq 'approve' ? 'display: inline-block;' : 'display: none;'}">임시저장</button>
			<button type="submit" id="approvalButton" name="action"
				value="approval"
				style="${(board.payOption eq 'save' or board.payOption eq 'reject' && (loggedInUserRank eq '사원' || loggedInUserRank eq '대리')) || (board.payOption eq 'wait' && loggedInUserRank eq '과장') || (board.payOption eq 'checking' && loggedInUserRank eq '부장') ? 'display: inline-block;' : 'display: none;'}">결재</button>
			<button type="submit" id="rejectButton" name="action" value="reject"
				style="${(board.payOption eq 'wait' && loggedInUserRank eq '과장') || (board.payOption eq 'checking' && loggedInUserRank eq '부장') ? 'display: inline-block;' : 'display: none;'}">반려</button>
